...
# from agents.report_generator import generate_scan_report
# from core.db import get_db
# from core.api_format_detector import JSONPathExtractor
...
#     "subprocess.run": ("HIGH", "subprocess.run() can execute arbitrary shell commands."),
#     "subprocess.Popen": ("HIGH", "subprocess.Popen() can execute arbitrary shell commands."),
#     "_import": ("MEDIUM", "import_() can dynamically load a module chosen at runtime."),
#     "pickle.loads": ("CRITICAL", "pickle.loads() on untrusted input can execute arbitrary code during deserialization."),
#     "yaml.load": ("HIGH", "yaml.load() without SafeLoader can execute arbitrary code from the YAML content."),
# }
...
#             if response_key_path:
#                 extracted = extractor.extract(data, response_key_path)
...
#                 auth_header=scan_config.get("auth_header"),
#                 request_format=scan_config.get("request_format"),
#                 response_key_path=scan_config.get("response_key_path"),
#             )
...
_DANGEROUS_CALL_SIGNATURES = {
    ...
    "subprocess.run": ("HIGH", "subprocess.run() can execute arbitrary shell commands."),
    "subprocess.Popen": ("HIGH", "subprocess.Popen() can execute arbitrary shell commands."),
    "import": ("MEDIUM", "import() can dynamically load a module chosen at runtime."),
    "pickle.loads": ("CRITICAL", "pickle.loads() on untrusted input can execute arbitrary code during deserialization."),
    "yaml.load": ("HIGH", "yaml.load() without SafeLoader can execute arbitrary code from the YAML content."),
}
...
#                 return f"ERROR: Empty response body (status {resp.status_code})"

#             try:
#                 data = resp.json()
#             except Exception:
#                 return f"ERROR: Non-JSON response (status {resp.status_code}): {resp.text[:200]}"
...
async def run_attack_on_endpoint(
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            if use_form_data:
                form_body = {**(form_extra_fields or {}), "prompt": payload}
                resp = await client.post(endpoint_url, data=form_body, headers=headers)
            else:
                headers["Content-Type"] = "application/json"
                body = request_format or {"messages": [{"role": "user", "content": payload}]}
                resp = await client.post(endpoint_url, json=body, headers=headers)
...
async def run_full_scan(agent_id: str, scan_config: dict) -> dict:
    ...
    async def run_single_attack(attack: dict):
        async with semaphore:
            response = await run_attack_on_endpoint(
                ...
                request_format=scan_config.get("request_format"),
                response_key_path=scan_config.get("response_key_path"),
                use_form_data=scan_config.get("use_form_data", False),
                form_extra_fields=scan_config.get("form_extra_fields"),
            )
    ...
    # ── Phase 3b: Multi-Turn Attack Chains ──────────────────────
    multiturn_results = []
    if endpoint_url and scan_config.get("run_multiturn", True):
        chains = get_all_chains()
        mt_semaphore = asyncio.Semaphore(2)
    ...
    # ── Phase 3c: Document / Indirect Injection ───────────────
    document_injection_results = []
    if endpoint_url and scan_config.get("run_document_injection", True):
        doc_payloads = get_all_document_payloads()
        doc_semaphore = asyncio.Semaphore(2)
            ...
            async with doc_semaphore:
                ...
                try:
                    ...
                    async with httpx.AsyncClient(timeout=30.0) as client:
                        ...
                        if resp.status_code >= 400:
                            return {**doc, "execution_failed": True, "error": f"HTTP {resp.status_code} — {resp.text[:200]}", "verdict": None}
                        if not resp.text.strip():
                            return {**doc, "execution_failed": True, "error": f"Empty response body (status {resp.status_code})", "verdict": None}
                        try:
                            data = resp.json()
                        except Exception:
                            return {**doc, "execution_failed": True, "error": f"Non-JSON response (status {resp.status_code}): {resp.text[:200]}", "verdict": None}
                        if "choices" in data:
                            agent_response = data["choices"][0]["message"]["content"]
                        elif "response" in data:
                            agent_response = data["response"]
    ...
    # Recalculate security_score to include multiturn + doc injection failures
    mt_executed = [r for r in multiturn_results if not r.get("execution_failed")]
    doc_executed = [r for r in document_injection_results if not r.get("execution_failed")]
    ...
    if all_total > 0:
        ...
        static_pen = sum(SEVERITY_PENALTY.get(f.get("severity"), 3) for f in static_findings)
        report["security_score"] = max(0, raw_pass_rate - static_pen)
        report["statistics"]["total_attacks_including_multiturn"] = all_total
        report["statistics"]["total_successful_including_multiturn"] = all_successful
