...
{{"executive_summary": "...", "risk_level": "CRITICAL_or_HIGH_or_MEDIUM_or_LOW", "top_fixes": [{{"priority": 1, "issue": "...", "fix": "...", "example": "..."}}]}}"""
    ...
    try:
        resp = await call_llm(
            ...
            temperature=0.2,
            max_tokens=MAX_TOKENS["report_summary"],
        )
        log_llm_call("report_summary", summary_prompt, resp)
        ai_summary = _strict_json_parse(resp)
        if ai_summary is None:
            print(f"[ReportGen] JSON parse FAILED. Raw LLM output was:\n{resp}")
    except Exception as e:
        print(f"[ReportGen] LLM call FAILED: {e}")
        ai_summary = None
    ...
    return {
        ...
        "statistics": {
            ...
            "failed_attacks": len(failed_attacks),
            "needs_review_attacks": len(needs_review_attacks),
            "pass_rate": f"{round(passed/total*100)}%" if total > 0 else "N/A (no dynamic test run)",
            "static_findings": len(static_findings),
            "critical_count": len(critical),
            "high_count": len(high),
        },
